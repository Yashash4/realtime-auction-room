...
--   1. is_public — public rooms are watchable by any logged-in user (and listed
--      in Live Now); private rooms are visible only to members (admin + joined
--      teams) in EVERY state. Tightens the 0009 active/paused watch access so it
--      opens only PUBLIC in-progress rooms to non-members.
--   2. max_players_per_team — optional squad-size cap (null/0 = no cap), enforced
--      in place_bid against already-SOLD players, inside the room lock.
-- Join-by-code is unaffected: join_room is SECURITY DEFINER and looks rooms up by
-- code regardless of RLS, so a private room's members still join from the lobby.

alter table public.rooms add column is_public boolean not null default true;
alter table public.rooms add column max_players_per_team integer;  -- null/0 = no cap

-- ---------------------------------------------------------------------------
-- Watch access. can_view_room gates items/bids/participants/events reads; the
-- in-progress branch now requires is_public, so a private room is readable only
-- by its admin and joined teams. Demo rooms stay visible to everyone.
-- ---------------------------------------------------------------------------
...
as $$
  select exists (
    ...
    where r.id = p_room
      and (
        r.is_demo
        or (r.is_public and r.status in ('active', 'paused'))
...
create policy rooms_select on public.rooms
  ...
  using (
    is_demo
    or (is_public and status in ('active', 'paused'))
    or admin_id = auth.uid()
    or is_room_participant(id)
  );

-- ---------------------------------------------------------------------------
-- place_bid: enforce the squad cap. A team one short can still bid on the
-- current item (winning it fills the squad); once it owns max_players_per_team
-- SOLD players, further bids are rejected. Checked inside the room lock, so the
-- count can't race a concurrent resolution.
-- ---------------------------------------------------------------------------
